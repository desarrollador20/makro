...
import { View, Text, SafeAreaView, ScrollView, LogBox, Alert, TouchableOpacity } from 'react-native';
...
import { stylesGlobal, theme, storageResult, checkConnected } from '../../../utils';
...
import { ButtonsQuestion, CheckboxCustom, Disagreed, MultimediaQuestion } from '../../../components/question';
...
export function QuestionScreen(props) {
  ...
  const validateDataInit = async () => {
    ...
    if (dataQuestion) {
      ...
      if (typeof DatosStorage !== undefined && DatosStorage !== null) {
        if (DatosStorage[keyQuestion] !== undefined && DatosStorage[keyQuestion] == '1') {
          setAgreed(true);
        }
        if (DatosStorage[keyQuestion] !== undefined && DatosStorage[keyQuestion] == '2') {
          setDisagreed(true);
        }
        if (DatosStorage[keyQuestion] !== undefined && DatosStorage[keyQuestion] == '3') {
  ...
  function UploadImage(props) {
    ...
    const imageSelect = async () => {
      ...
      if (status === "denied") {
        Toast.show({
          ...
          position: "bottom",
          text1: 'Advertência',
          text2: 'É necessário aceitar as permissões da galeria para carregar imagens. '
        });
      } else {
        if (imageSelected.length >= 3) {
          return (Toast.show({
            ...
            position: "bottom",
            text1: 'Advertência',
            text2: 'só se pode carregar 3 imagens por pergunta'
          }))
        }
        ...
        if (result.cancelled) {
          Toast.show({
            ...
            position: "bottom",
            text1: 'Advertência',
            text2: 'Encerrou sem escolher uma imagem'
          })
        } else {
          let dataImage = result.uri?.split('/');
          let nameImage = dataImage[dataImage?.length - 1];
          storageResult.setItemValueImages(route.params.idCheckList, route.params.id, dataQuestion.id, nameImage, result.base64);
    ...
    const removeImage = (image) => {
      ...
      Alert.alert(
        "Eliminar imagem",
        "¿Tem a certeza que pretende apagar a imagem?",
        [
          {
            text: "Cancelar",
            style: "cancel"
                ...
                Object.entries(DatosStorageImages).forEach(([key, value]) => {
                  if (key.includes(keyQuestionImages)) {
                    if (value === image) {
                      ...
                      storageResult.setItemValueImages(dataKey[0], dataKey[1], dataKey[2], dataKey[3], 'deleted');
                    } else {
                      if (value !== 'deleted') {
                        DataImages.push(value);
                      }
    ...
    return (
      <>
        ...
        <TouchableOpacity style={styles.viewImage} onPress={imageSelect}>
          ...
          />
          <Text style={styles.lblUploadImage}>carregar provas fotográficas</Text>
        </TouchableOpacity>

        <View style={styles.containerImagesUpload}>
          {imageSelected && imageSelected.map((item, i) => {
            ...
            <>
              <ScrollView>
                ...
                <View style={{ ...stylesGlobal.contentView, }}>
                  <Text style={styles.lblQuestion}>{(language.current == 'pt') ? dataQuestion.titlePortuguese : dataQuestion.titleSpanish}</Text>
                  {connectStatus && (
                    <MultimediaQuestion
                  ...
                  <View style={styles.container}>
                    <CheckboxCustom
                      ...
                      title={(language.current == 'pt') ? dataQuestion.answer_options[0].labelPortuguese : dataQuestion.answer_options[0].labelSpanish}
                    ...
                    <CheckboxCustom
                      ...
                      title={(language.current == 'pt') ? dataQuestion.answer_options[1].labelPortuguese : dataQuestion.answer_options[1].labelSpanish}
                    ...
                    <CheckboxCustom
                      ...
                      title={(language.current == 'pt') ? dataQuestion.answer_options[2].labelPortuguese : dataQuestion.answer_options[2].labelSpanish}
                      toDisabledOne={setAgreed}
