...
export function useClickButtom() {
    ...
    const managerScreenQuestion = async (idCategory, numberQuestion, idQuestion, screenPositiveComments = 0, screenResponsileList = 0, idCheckList, action, numberOption) => {
        ...
        let currentNumberQuestion = (action == 'next') ? (parseInt(numberQuestion) + 1) : (parseInt(numberQuestion) - 1);

        //if you enter here is because you have run out of questions or you are returning to the category menu
        if (DatosStorage['dataQuestions']['questions_' + idCategory]['data']['orden_' + currentNumberQuestion] === undefined) {
            if (screenPositiveComments == 1 && action == 'next' && connectStatus) {
                ...
                Object.entries(StorageResponse).forEach(([key, value]) => {
                    if (key.includes(idCategory) && key.includes('checkboxSelected') && value == '2') {
                        disagreed = 1;
                        return;
                    }
                });
                ...
                if (disagreed == 1) {
                    return navigation.navigate(
                        screen.responsibleList.tab, {
                        screen: screen.responsibleList.responsibleList,
                        params: { id: idCategory, numberQuestion: numberQuestion, idCheckList }
                    });
                }
            ...
            if (screenPositiveComments == 1 && action == 'prev') {
                return navigation.navigate(screen.question.tab, {
                    ...
                    params: {
                        id: idCategory, numberQuestion: numberQuestion, idCheckList,
                    }
                });
            }
            if (screenResponsileList == 1 && action == 'prev') {
                return navigation.navigate(
                    screen.positiveComments.tab, {
                    screen: screen.positiveComments.positiveComments,
                    params: { id: idCategory, numberQuestion: currentNumberQuestion, idCheckList }
                });
            }
        ...
        return navigation.navigate(screen.question.tab, {
            screen: screen.question.question,
            params: {
                id: idCategory, numberQuestion: currentNumberQuestion, idCheckList
            }
        });
    ...
    const validarFields = async (idCategory, numberQuestion, idQuestion, screenPositiveComments = 0, screenResponsileList = 0, idCheckList, action) => {
        ...
        if (screenPositiveComments == 0 && screenResponsileList == 0) {
            if (DatosStorage === null) {
                return Toast.show({
                    type: "error",
                    position: "bottom",
                    text1: "Dados ausentes",
                    text2: "Selecione uma opção antes de continuar",
                });
            }
            if (typeof DatosStorage !== undefined) {
                ...
                if (DatosStorage[keyQuestion] == '2') {
                    if (DatosStorage[keyObservation] === undefined || DatosStorage[keyObservation] == '') {
                        return Toast.show({
                            type: "error",
                            position: "bottom",
                            text1: "Dados ausentes",
                            text2: "Insira a observação para a não conformidade",
                        });
                    }
                    if (DatosStorage[keyProposedMeasures] === undefined || DatosStorage[keyProposedMeasures] == '') {
                        return Toast.show({
                            type: "error",
                            position: "bottom",
                            text1: "Dados ausentes",
                            text2: "Inserir as medidas propostas para a não conformidade",
                        });
                    }
                }
            }
        ...
        return managerScreenQuestion(idCategory, numberQuestion, idQuestion, screenPositiveComments, screenResponsileList, idCheckList, action, 0);
